...
async function callTrendingMoviesA_DayBanner() {
...
async function trendingMoviesA_DayBanner() {
  const trendingMoviesA_Day = await callTrendingMoviesA_DayBanner();
  ...
  for (let i = 0; i < 1; i++) {
    ...
    story.textContent = trendingMoviesA_Day[randomIndices[i]].overview;
...
async function moviesTrailer(movieId) {
...
async function trailer(videoId) {
  // alert("Hello World!");
...
trendingMoviesA_DayBanner();

async function trendingMoviesSlide() {
  const trending = await callTrendingMoviesSlide();
